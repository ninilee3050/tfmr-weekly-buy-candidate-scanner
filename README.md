...
다만 기존 코드와 CSV 파일명에는 구현 이력상 `buy_points`, `매수포인트`, `기준봉` 표현이 일부 남아 있을 수 있습니다. 이 표현들은 확정 수익 지점이 아니라, TFMR 시나리오 조건에 맞는 후보 지점을 의미합니다.
...
화면 없이 Top100 자동 스캔만 실행하려면 아래 명령을 사용합니다.
...
* 이 구현은 MMRM 레포 `ninilee3050/mmrm-weekly-buy-candidate-scanner`의 Top100 방식과 맞춘 것입니다.
...
* 참고 레포 `ninilee3050/mmrm-weekly-buy-candidate-scanner`의 `market_cap_provider.py`와 비교하면 됩니다.
...
```text
f80b4f3 Initial TFMR buy candidate scanner
```
